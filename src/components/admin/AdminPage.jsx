...
const transformApiProduct = (apiProduct) => {
    ...
    const images = apiProduct.image_urls || [];
...
const OrderDetailModal = ({ order, isDarkMode, onClose }) => ( <> <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[2000]" onClick={onClose} /> <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} exit={{ scale: 0.8 }} className={`fixed inset-0 m-auto h-fit max-h-[90vh] overflow-y-auto max-w-lg lg:max-w-4xl p-8 rounded-3xl shadow-2xl z-[2001] ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`} onClick={e => e.stopPropagation()}><div className="flex justify-between items-center border-b pb-4 mb-6 border-gray-700/50"><h3 className="text-3xl font-bold text-pink-500">Order #{order.id} Details</h3><button onClick={onClose} className="p-2 rounded-full hover:bg-gray-700/50"><X className="w-6 h-6" /></button></div><div className="grid grid-cols-1 lg:grid-cols-2 gap-8"><div className="space-y-4 p-4 rounded-xl dark:bg-gray-700/50 bg-gray-50 shadow-inner"><h4 className="text-xl font-bold mb-3 flex items-center gap-2 text-purple-500"><Users className="w-5 h-5" /> Customer Info</h4><p>Name: {order.customer}</p><p>Address: {order.address}</p></div><div className="space-y-4"><h4 className="text-xl font-bold mb-3 flex items-center gap-2 text-pink-500"><ShoppingBag className="w-5 h-5" /> Items ({order.items.length})</h4><ul>{order.items.map((item, index) => <li key={index} className="flex justify-between p-2 rounded-md dark:bg-gray-700 bg-gray-100"><span>{item.name} x {item.qty}</span><span>₹{(item.price * item.qty).toFixed(2)}</span></li>)}</ul><div className="pt-2 border-t dark:border-gray-700"><p className="text-lg font-bold flex justify-between">Total: <span className="text-pink-500">₹{order.total.toFixed(2)}</span></p></div></div></div></motion.div></> );
const CouponFormModal = ({ onClose, onSave, isDarkMode }) => { const [formData, setFormData] = useState({ code: '', discount: 10, usageLimit: 100, minOrderValue: 0 }); const handleChange = (e) => setFormData(p => ({ ...p, [e.target.name]: e.target.type === 'number' ? Number(e.target.value) : e.target.value.toUpperCase().replace(/\s/g, '') })); const handleSubmit = (e) => { e.preventDefault(); if (!formData.code) return; onSave(formData); onClose(); }; return ( <> <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/70 z-[2000]" onClick={onClose} /> <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} exit={{ scale: 0.8 }} className={`fixed inset-0 m-auto h-fit max-w-lg p-8 rounded-3xl shadow-2xl z-[2001] ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`} onClick={e => e.stopPropagation()}><h3 className="text-2xl font-bold mb-6">Request New Coupon</h3><form onSubmit={handleSubmit} className="space-y-4"><div><label>Coupon Code</label><input type="text" name="code" value={formData.code} onChange={handleChange} className={`w-full p-2 rounded-lg border dark:bg-gray-700 dark:border-gray-600`} required /></div><div><button type="submit" className="w-full py-3 bg-purple-500 hover:bg-purple-600 text-white font-bold rounded-xl">Submit for Approval</button></div></form></motion.div></> ); };
...
const Dashboard = ({ products, orders, users, setActiveSection, userRole }) => {
    ...
    return ( <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-10"><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">{[...financeCards, ...operationalCards].map((card, i) => <DashboardCard key={i} {...card} onClick={() => setActiveSection(card.section)} />)}</div><div className="grid grid-cols-1 lg:grid-cols-2 gap-8"> <div className="p-6 rounded-2xl bg-white dark:bg-gray-800 shadow-lg"><h3 className="text-xl font-bold mb-4 text-orange-500">Low Stock Items</h3><ul>{lowStockItems.map(p => (<li key={p.id} className="flex justify-between items-center text-sm p-2 rounded-lg bg-gray-100 dark:bg-gray-700"><span>{p.name}</span><span className={`font-semibold ${p.stock === 0 ? 'text-red-500' : 'text-orange-500'}`}>Stock: {p.stock}</span></li>))}</ul></div><div className="p-6 rounded-2xl bg-white dark:bg-gray-800 shadow-lg"><h3 className="text-xl font-bold mb-4 text-cyan-500">Recent Orders</h3><ul>{orders.slice(0, 3).map(o => (<li key={o.id} className="flex justify-between p-2 rounded-lg bg-gray-100 dark:bg-gray-700 text-sm"><span>Order {o.id} ({o.customer})</span><span className="font-semibold">{o.status}</span></li>))}</ul></div></div></motion.div> );
};

// --- MODIFIED: ProductFormModal ---
const ProductFormModal = ({ onClose, isDarkMode, product, onSave }) => {
    const isEditing = !!product;
    const AVAILABLE_SIZES = useMemo(() => ['1y-2y', '2y-3y', '3y-4y', '4y-5y', '5y-6y', '6y-7y', '7y-8y', '8y-9y', '9y-10y', '10y-11y', '11y-12y', '12y-13y'], []);
    const [formData, setFormData] = useState(product ? { ...product, originalPrice: product.originalPrice || product.price, discountedPrice: product.discountedPrice || product.price } : { id: null, name: '', description: '', category: 'Tops', originalPrice: 0, discountedPrice: 0 });
...
    const handleChange = (e) => setFormData(p => ({ ...p, [e.target.name]: e.target.type === 'number' ? Number(e.target.value) : e.target.value }));
    ...
    const handleAddVariant = () => {
        ...
        setVariants(prev => [...prev, { ...currentVariant, imageUrls: [currentVariant.imageUrl] }]);
...
    const handleSubmit = (e) => { e.preventDefault(); onSave({ ...formData, variants }); onClose(); };
...
                </div>
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div><label className="block font-semibold">Original Price (₹)</label><input type="number" name="originalPrice" value={formData.originalPrice} onChange={handleChange} className={`w-full p-2 rounded-lg border dark:bg-gray-700 dark:border-gray-600`} required /></div>
                        <div><label className="block font-semibold">Discounted Price (₹)</label><input type="number" name="discountedPrice" value={formData.discountedPrice} onChange={handleChange} className={`w-full p-2 rounded-lg border dark:bg-gray-700 dark:border-gray-600`} /></div>
                    </div>
                    ...
                    <div className="space-y-4 pt-4 border-t dark:border-gray-700">
                        ...
                        <div className="p-4 rounded-lg border dark:border-gray-600 space-y-3">
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
                                ...
                                <div><label className="text-sm font-medium">Stock</label><input type="number" name="stockQuantity" value={currentVariant.stockQuantity} onChange={handleVariantChange} className="w-full p-2 rounded-lg border dark:bg-gray-700 dark:border-gray-600 text-sm" /></div>
                                <div className="md:col-span-4"><label className="text-sm font-medium">Image URL</label><input type="text" name="imageUrl" value={currentVariant.imageUrl} onChange={handleVariantChange} placeholder="/images/tshirt_red.jpg" className="w-full p-2 rounded-lg border dark:bg-gray-700 dark:border-gray-600 text-sm" /></div>
                            </div>
                            <button type="button" onClick={handleAddVariant} className="w-full py-2 bg-green-500 text-white rounded-xl font-bold flex items-center justify-center"><Plus className="w-5 h-5 mr-2" /> Add Variant</button>
                        </div>
                        
                        <div className="space-y-2 max-h-40 overflow-y-auto">
                            <h5 className="font-semibold">Added Variants ({variants.length})</h5>
                            {variants.map((v, index) => (
                                <div key={index} className="flex justify-between items-center p-2 bg-gray-100 dark:bg-gray-700 rounded-md text-sm">
                                    <span>{AVAILABLE_COLORS.find(c=>c.id === v.colorId)?.name} / {AVAILABLE_SIZES_MAP.find(s=>s.id === v.sizeId)?.name}</span>
                                    <span>Stock: {v.stockQuantity}</span>
                                    <button type="button" onClick={() => handleRemoveVariant(index)} className="text-red-500"><Trash2 className="w-4 h-4" /></button>
                                </div>
                            ))}
                        </div>
                    </div>

                    <div className="col-span-1 lg:col-span-2 pt-4 border-t dark:border-gray-700"><button type="submit" className="w-full py-3 bg-pink-500 text-white font-bold rounded-xl"><Save className="w-5 h-5 mr-2 inline" /> {isEditing ? 'Save Changes' : 'Create Product'}</button></div>
                </form>
            </motion.div>
        </>
    );
...
// --- MODIFIED: ProductManagement Section ---
const ProductManagement = ({ products, setProducts, isDarkMode, cardBaseClasses, userRole }) => {
    const [activeTab, setActiveTab] = useState('products');
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [productToEdit, setProductToEdit] = useState(null);
...
    };
    ...
    // ✅ FIX: `handleProductSave` now sends JSON payload that matches Postman
    const handleProductSave = async (formData) => {
        const isEditing = !!formData.id;
        const authToken = localStorage.getItem('adminToken');
        if (!authToken) return alert('Authentication error. Please log in again.');

        // --- 🔴 FIX START: Create payload that matches Postman request ---
        const productPayload = {
            name: formData.name,
            description: formData.description,
            original_price: Number(formData.originalPrice),
            price: Number(formData.discountedPrice), // Your backend might use this for discounted price
            category: formData.category,
            variants: formData.variants, // The variants array is now managed in the form state
        };
        // --- 🔴 FIX END ---
        
        try {
            const response = await fetch(`${API_BASE_URL}/products${isEditing ? `/${formData.id}` : ''}`, {
            ...
            if (!response.ok) {
                ...
                try {
                    ...
                    if (errorText.includes('<!DOCTYPE html>')) {
                        const match = errorText.match(/<pre>Error: (.*?)<br>/);
                        throw new Error(match ? match[1] : 'An unknown server error occurred.');
                    }
            ...
            const responseData = await response.json();
            const savedProduct = transformApiProduct(responseData.data);
...
            <AnimatePresence>{isModalOpen && <ProductFormModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} isDarkMode={isDarkMode} product={productToEdit} onSave={handleProductSave} />}</AnimatePresence>
            <div className="flex space-x-4 border-b border-gray-700/50">
                <button onClick={() => setActiveTab('products')} className={`pb-2 font-semibold transition-colors ${activeTab === 'products' ? 'text-pink-500 border-b-2 border-pink-500' : 'text-gray-500 hover:text-white'}`}>Product Inventory</button>
                {(userRole === 'admin' || userRole === 'product_manager') && <button onClick={() => setActiveTab('coupons')} className={`pb-2 font-semibold transition-colors ${activeTab === 'coupons' ? 'text-pink-500 border-b-2 border-pink-500' : 'text-gray-500 hover:text-white'}`}>Coupon Management</button>}
            </div>
            {activeTab === 'products' && (
                <div className="space-y-6">
                    {canEditProducts && <button className="px-6 py-3 bg-green-500 hover:bg-green-600 text-white rounded-xl font-bold transition-colors" onClick={() => handleAction('Add', null)}>+ Add New Product</button>}
...
const OrderManagement = ({ orders, setOrders, setRefundQueue, isDarkMode, cardBaseClasses, userRole }) => { const canManageOrders = userRole === 'admin' || userRole === 'product_manager'; const [selectedOrder, setSelectedOrder] = useState(null); return ( <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="space-y-6"><AnimatePresence>{selectedOrder && <OrderDetailModal order={selectedOrder} isDarkMode={isDarkMode} onClose={() => setSelectedOrder(null)} />}</AnimatePresence><h3 className={`text-2xl font-bold`}>Order List</h3><div className={`overflow-x-auto ${cardBaseClasses} rounded-xl shadow-lg`}><table className="min-w-full divide-y dark:divide-gray-700"><thead className="bg-gray-100 dark:bg-gray-700"><tr><th>Order ID</th><th>Customer</th><th>Total</th><th>Status</th></tr></thead><tbody className="divide-y dark:divide-gray-700">{orders.map((o) => (<tr key={o.id}><td className="px-6 py-4 font-semibold text-pink-500 cursor-pointer" onClick={() => setSelectedOrder(o)}>{o.id}</td><td>{o.customer}</td><td>₹{o.total.toFixed(2)}</td><td>{o.status}</td></tr>))}</tbody></table></div></motion.div> ); };
const FinanceManagement = ({ isDarkMode, cardBaseClasses, userRole, refundQueue, setRefundQueue }) => { return ( <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }}><h3 className="text-2xl font-bold">Financial Overview</h3></motion.div> ); };
const CouponManagement = ({ isDarkMode, cardBaseClasses, userRole }) => { const [isCouponModalOpen, setIsCouponModalOpen] = useState(false); const [myCoupons, setMyCoupons] = useState([]); return ( <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }}><button className="px-6 py-3 bg-purple-500 text-white rounded-xl font-bold" onClick={() => setIsCouponModalOpen(true)}>+ Request New Coupon</button></motion.div> ); };
const UserManagement = ({ users, isDarkMode, cardBaseClasses }) => ( <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }}><h3 className="text-2xl font-bold">Customer List</h3></motion.div> );
...
        } catch (error) {
            console.error('Error updating offer:', error);
            alert(`Could not save offer. Reason: ${error.message}`);
        }
    };
...
                // Correctly access the nested data array and transform it
                if (Array.isArray(resData.data)) {
                    setProducts(resData.data.map(transformApiProduct));
                } else {
                    setProducts([]);
                }
            } catch (error) {
...
                if (!response.ok) throw new Error('Could not fetch active offer.');
                const data = await response.json();
                if (data && data.data && data.data.description) {
                    setOfferText(data.data.description);
                } else {
                    setOfferText('No active offer found.'); 
                }
            } catch (error) {
                console.error("Failed to fetch offer:", error);
                setOfferText('Could not load offer.');
            } finally {
                setIsLoading(false);
            }
        };
...
            <div className="flex">
                {/* Mobile Sidebar */}
               <motion.div initial={false} animate={isSidebarOpen ? { x: 0 } : { x: '-100%' }} transition={{ ease: "easeInOut" }} className={`fixed top-0 left-0 h-full w-64 p-6 flex flex-col z-30 lg:hidden ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}>
                    <button onClick={() => setIsSidebarOpen(false)} className="absolute top-4 right-4 p-2 rounded-full hover:bg-gray-700/50"><X className="w-6 h-6" /></button>
                    <h1 className={`text-2xl font-extrabold mb-8 bg-gradient-to-r ${isDarkMode ? 'from-purple-400 to-cyan-400' : 'from-pink-500 to-purple-600'} bg-clip-text text-transparent`}>{userRole?.role_name?.toUpperCase().replace('_', ' ') || 'MANAGER'}</h1>
                    <nav className="space-y-2 flex-grow">
                        {filteredNavItems.map(item => <button key={item.id} onClick={() => handleSectionChange(item.id)} className={`w-full text-left flex items-center p-3 rounded-xl transition-all ${activeSection === item.id ? 'bg-pink-500 text-white shadow-lg' : 'hover:bg-gray-700/50 hover:text-white'}`}><item.icon className="w-5 h-5 mr-3" />{item.label}</button>)}
                    </nav>
                </motion.div>
...
                    <p className="hidden sm:block text-sm">Logged in as: <span className="font-semibold capitalize">{userRole?.role_name}</span></p>
                </header>
                {isLoading ? <p>Loading content...</p> : renderSection()}
            </main>
        </div>
    );
};
