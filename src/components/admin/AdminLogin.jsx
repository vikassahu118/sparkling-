...
import { LogIn, User, Lock } from 'lucide-react'; // ✅ CHANGED: Replaced Mail with User icon

const API_BASE_URL = 'http://192.168.31.3:8000/api/v1';

const AdminLogin = ({ isDarkMode, onLoginSuccess }) => {
    // ✅ CHANGED: State renamed to 'emailOrMobile'
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // ✅ CHANGED: Check for user and token inside the nested 'data' property
            if (data.data && data.data.user && data.data.accessToken) {
                // ✅ CHANGED: Pass the nested data object to the success handler
    ...
    return (
        <motion.div
            ...
            className={`py-20 max-w-md mx-auto p-8 rounded-3xl shadow-2xl my-12 ${isDarkMode ? 'bg-gray-800 text-white shadow-purple-900/40' : 'bg-white text-gray-900 shadow-pink-300/80'
                }`}
        >
            <h2 className={`text-3xl font-extrabold mb-2 text-center bg-gradient-to-r ${isDarkMode ? 'from-purple-400 to-cyan-400' : 'from-pink-500 to-purple-600'
